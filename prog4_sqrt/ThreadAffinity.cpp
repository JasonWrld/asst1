...
bool queryUsableCpuSets(
    ...
    while (offset < returnedLength) {
        ...
        if (information->Size == 0 ||
            information->Size > returnedLength - offset) {
        ...
        if (information->Type == CpuSetInformation &&
            ...
            minimumEfficiency = std::min(
                minimumEfficiency, target.efficiencyClass);
            maximumEfficiency = std::max(
                maximumEfficiency, target.efficiencyClass);
        }
...
bool queryProcessDefaultCpuSetIds(
    ...
    if (!GetProcessDefaultCpuSets(
            ...
            &requiredCount)) {
        error = windowsErrorMessage(
            "GetProcessDefaultCpuSets", GetLastError());
        return false;
    }
...
bool queryCurrentThreadCpuSetIds(
    ...
    if (!GetThreadSelectedCpuSets(
            ...
            &requiredCount)) {
        error = windowsErrorMessage(
            "GetThreadSelectedCpuSets", GetLastError());
        return false;
    }
    windowsIds.resize(requiredCount);
    ids.assign(windowsIds.begin(), windowsIds.end());
...
bool setProcessDefaultCpuSetIds(
    ...
    if (!SetProcessDefaultCpuSets(
            ...
            static_cast<ULONG>(windowsIds.size()))) {
        error = windowsErrorMessage(
            "SetProcessDefaultCpuSets", GetLastError());
        return false;
    }
    return true;
}
...
bool setCurrentThreadCpuSetIds(
    ...
    if (!SetThreadSelectedCpuSets(
            ...
            static_cast<ULONG>(windowsIds.size()))) {
        error = windowsErrorMessage(
            "SetThreadSelectedCpuSets", GetLastError());
        return false;
    }
    return true;
}
...
bool buildMyth4AffinityPlan(
    ...
    std::string& error) {
    plan = Myth4AffinityPlan{};
    error.clear();

#ifndef _WIN32
    error = "--simulate-myth4 is supported only by the native Windows build; "
            "WSL does not expose reliable host P-core topology.";
    return false;
#else
    constexpr std::size_t mythPhysicalCores = 4;
    constexpr std::size_t contextsPerCore = 2;
    ...
    if (minimumEfficiency == maximumEfficiency) {
        error = "Windows did not expose distinct efficiency classes, so "
                "P-cores cannot be identified safely.";
        return false;
    }
    ...
    selectedCores.reserve(mythPhysicalCores);
    for (auto& entry : performanceCores) {
        ...
        if (contexts.size() >= contextsPerCore) {
            selectedCores.push_back(contexts);
        }
        if (selectedCores.size() == mythPhysicalCores) {
            break;
        }
    }
    ...
    if (selectedCores.size() != mythPhysicalCores) {
        std::ostringstream output;
        output << "Myth4 simulation requires four physical P-cores with two "
               << "SMT contexts each, but Windows exposed only "
               << selectedCores.size() << " suitable core(s).";
        error = output.str();
        plan = Myth4AffinityPlan{};
        return false;
    }

    // Spread contexts 0-3 across four physical cores, followed by their SMT
    // siblings in contexts 4-7.
    for (std::size_t sibling = 0; sibling < contextsPerCore; ++sibling) {
        for (const std::vector<ThreadAffinityTarget>& core : selectedCores) {
            plan.logicalProcessors.push_back(core[sibling]);
        }
    }
    plan.selectedPhysicalCoreCount = mythPhysicalCores;
    return true;
#endif
}
...
void printMyth4AffinityPlan(const Myth4AffinityPlan& plan) {
    ...
    if (!plan.logicalProcessors.empty()) {
        const ThreadAffinityTarget& reference = plan.logicalProcessors.front();
        std::printf(
            "  Serial/ISPC reference -> CPU Set %u, Group %u, "
            "Logical CPU %u, CoreIndex %u\n",
            static_cast<unsigned int>(reference.cpuSetId),
            static_cast<unsigned int>(reference.group),
            static_cast<unsigned int>(reference.logicalProcessorIndex),
            static_cast<unsigned int>(reference.coreIndex));
    }
    std::printf(
        "  ISPC tasks -> Windows scheduling within the 8 myth4 contexts\n");
}
...
bool applyProcessCpuSetRestriction(
    ...
    const std::set<std::uint32_t> uniqueIds(
    ...
    if (!setCurrentThreadCpuSetIds(requestedIds, error)) {
        ...
        if (!restoreProcessCpuSetRestriction(state, rollbackError)) {
            error = assignmentError + "; rollback also failed: " +
                    rollbackError;
        } else {
    ...
    if (!queryProcessDefaultCpuSetIds(actualProcessIds, error) ||
        ...
        std::string rollbackError;
        if (!restoreProcessCpuSetRestriction(state, rollbackError)) {
            error = verificationError + "; rollback also failed: " +
                    rollbackError;
        } else {
...
bool bindCurrentThread(
    ...
    if (target.logicalProcessorIndex >= maskBits) {
        error = "Logical processor index cannot be represented by "
                "GROUP_AFFINITY.";
        return false;
    }
    ...
    requested.Mask =
        static_cast<KAFFINITY>(1) << target.logicalProcessorIndex;

    GROUP_AFFINITY previous{};
    if (!SetThreadGroupAffinity(GetCurrentThread(), &requested, &previous)) {
        error = windowsErrorMessage(
            "SetThreadGroupAffinity", GetLastError());
        return false;
    }
    ...
    if (!GetThreadGroupAffinity(GetCurrentThread(), &actual)) {
        error = windowsErrorMessage(
            "GetThreadGroupAffinity", GetLastError());
    ...
    if (actual.Group != requested.Group || actual.Mask != requested.Mask) {
        error = "Windows did not apply the requested one-processor "
                "affinity mask.";
        GROUP_AFFINITY ignored{};
        SetThreadGroupAffinity(GetCurrentThread(), &previous, &ignored);
        state.active = false;
...
bool restoreCurrentThreadAffinity(
    ...
    if (!SetThreadGroupAffinity(GetCurrentThread(), &previous, &ignored)) {
        error = windowsErrorMessage(
            "SetThreadGroupAffinity(restore)", GetLastError());
        return false;
    }
    state.active = false;
    return true;
...
bool configureMyth4TaskRuntime(std::string& error) {
    error.clear();
#ifndef _WIN32
    error = "The myth4 Concurrency Runtime policy is available only on "
            "Windows.";
    return false;
#else
    constexpr unsigned int mythExecutionContexts = 8;
    try {
        const concurrency::SchedulerPolicy policy(
            2,
            concurrency::MinConcurrency,
            mythExecutionContexts,
            concurrency::MaxConcurrency,
            mythExecutionContexts);
        concurrency::Scheduler::SetDefaultSchedulerPolicy(policy);
    } catch (const std::exception& exception) {
        error = std::string("Unable to configure the Concurrency Runtime: ") +
                exception.what();
        return false;
    }
    std::printf("Configured ConCRT concurrency resources: %u\n",
                mythExecutionContexts);
    return true;
#endif
}
